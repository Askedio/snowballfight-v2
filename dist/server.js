...
io.on("connection", (socket) => {
    ...
    players[socket.id] = { id: socket.id, x: 400, y: 300, health: 100, activeInputs: new Set(), };
...
setInterval(() => {
    ...
    Object.values(players).forEach((player) => {
        ...
        if (player.activeInputs.has("left"))
            player.x -= moveDistance;
        if (player.activeInputs.has("right"))
            player.x += moveDistance;
        if (player.activeInputs.has("up"))
            player.y -= moveDistance;
        if (player.activeInputs.has("down"))
            player.y += moveDistance;
    });
