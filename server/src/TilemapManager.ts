...
export interface TileBounds {
  ...
  export class TilemapManager {
    ...
    constructor(mapFilePath: string, collisionLayerName: string, spawnLayerName: string) {
      this.collisionIndex = new RBush<CollisionTile>();
  
      // Load and parse the map JSON
      const mapJson = JSON.parse(fs.readFileSync(mapFilePath, "utf8")) as tiled.Map;
  
      // Extract collision layer
      const collisionLayer = mapJson.layers.find((layer) => layer.name === collisionLayerName);
      if (!collisionLayer || collisionLayer.type !== "tilelayer") {
        throw new Error(`Collision layer "${collisionLayerName}" not found or is not a tile layer!`);
      }
      ...
      collisionLayer.data.forEach((tileIndex, index) => {
        if (tileIndex !== 0) { // Non-zero tile index indicates a collision tile
      ...
      // Extract spawn layer
      const spawnLayer = mapJson.layers.find((layer) => layer.name === spawnLayerName);
      if (!spawnLayer || spawnLayer.type !== "tilelayer") {
        throw new Error(`Spawn layer "${spawnLayerName}" not found or is not a tile layer!`);
      }
  
      // Parse spawn tiles
      spawnLayer.data.forEach((tileIndex, index) => {
        if (tileIndex !== 0) { // Non-zero tile index indicates a spawn tile
          const x = (index % mapJson.width) * this.tileWidth;
          const y = Math.floor(index / mapJson.width) * this.tileHeight;
    ...
    getRandomSpawn(): { x: number; y: number } {
      ...
      const randomTile = this.spawnTiles[Math.floor(Math.random() * this.spawnTiles.length)];
      return {
